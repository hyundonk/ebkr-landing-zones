...
module "virtual_network" {
  ...
  virtual_network_rg                = local.resource_group_hub_names["resourcegroup_name_network"]
  prefix                            = local.prefix
  location                          = local.location_map["region1"]
...
module "network_monitoring" {
  ...
  diag_storage_account_name         = local.diagnostics_map.diags_sa_name
  diag_storage_account_access_key   = local.diagnostics_map.diags_sa_access_key

  log_analytics_workspace_id        = local.log_analytics_workspace.workspace_id
  log_analytics_workspace_key       = local.log_analytics_workspace.key
}
