...
export TF_VAR_lowerlevel_adminusername=$(az keyvault secret show -n adminusername --vault-name ${keyvault} | jq -r .value)
export TF_VAR_lowerlevel_adminpassword=$(az keyvault secret show -n adminpassword --vault-name ${keyvault} | jq -r .value)
